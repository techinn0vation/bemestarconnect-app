import styled from "styled-components/native";
import { theme } from "../../themes";

export const ContentFormSteps = styled.View`
  width: 100%;
  height: auto;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  padding: ${theme.size(15)};
`;
export const ViewFormSteps = styled.View`
  width: 100%;
  height: auto;
  background: ${theme.colors.cinzaEscuro};
  border-radius: ${theme.size(10)};
  padding: ${theme.size(15)};

  flex-direction: column;
  justify-content: center;
  align-items: center;

  position: fixed;
  top: ${theme.size(-130)};
`;
export const FrameSteps = styled.Image`
  width: ${theme.size(100)};
  height: ${theme.size(100)};
  position: absolute;
  top: ${theme.size(-100)};
  left: ${theme.size(20)};
`;
export const MessageFormSteps = styled.View`
  width: auto;
  background: ${theme.colors.Azul};
  border-top-left-radius: ${theme.size(6)};
  border-top-right-radius: ${theme.size(6)};
  border-bottom-right-radius: ${theme.size(6)};

  justify-content: center;
  align-items: center;
  padding: ${theme.size(10)};
  position: absolute;
  top: ${theme.size(-130)};
  left: ${theme.size(115)};
`;
export const InnerMessageFormSteps = styled.Text`
  color: ${theme.colors.Branco};
  font-size: ${theme.size(13)};
  line-height: ${theme.size(20)};
  font-weight: 600;
  text-align: center;
`;
export const SubFraseFormSteps = styled.Text`
  color: ${theme.colors.Branco};
  font-size: ${theme.size(13)};
  line-height: ${theme.size(20)};
  font-weight: 600;
  text-align: center;
`;
export const ContentInputFormSteps = styled.View`
  width: 100%;
  flex-direction: row;
  justify-content: center;
  align-items: center;
`;
export const LateralTextFormSteps = styled.Text`
  color: ${theme.colors.Branco};
  font-size: ${theme.size(16)};
  font-weight: 600;
  line-height: ${theme.size(24)};
  text-align: center;
`;
export const InnerInputFormSteps = styled.TextInput`
  width: ${theme.size(90)};
  padding: ${theme.size(6)};
  border: solid ${theme.size(0.9)} ${theme.colors.Azul};
  border-radius: ${theme.size(6)};

  color: ${theme.colors.Branco};
  font-size: ${theme.size(14)};
  font-weight: 600;
  line-height: ${theme.size(21)};
  text-align: center;
`;
export const ButtonFormSteps = styled.TouchableOpacity`
  width: auto;
  padding: ${theme.size(10)};
  border-radius: ${theme.size(5)};
  background: ${theme.colors.Azul};
`;
export const TextButtonFormSteps = styled.Text`
  color: ${theme.colors.Branco};
  font-size: ${theme.size(14)};
  font-weight: 600;
  line-height: ${theme.size(21)};
  text-align: center;
  text-transform: capitalize;
`;
export const DividingLineFormSteps = styled.View`
  padding: ${theme.size(6)};
`;
